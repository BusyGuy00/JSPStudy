...
public class CaPaging {
	public static String pagingStr(int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl,String ca) {
		...
		if(pageTemp != 1) { 
			pagingStr += "<a href='" +reqUrl+ "?pageNum=1&ca="+ca+"'>[첫페이지]</a>";
			pagingStr  += "<a href='"+reqUrl+"?pageNum="+(pageTemp-1)+"&ca="+ca+"'>[이전블록]</a>";
		}
		//각 페이지 번호 출력 
		...
		while(blockcount <= blockPage && pageTemp <= totalpages) {
			...
			if(pageTemp == pageNum) {
				...
				pagingStr += "<a href='"+reqUrl+"?pageNum="+pageTemp+"&ca="+ca+"'>"+pageTemp+"</a>";
			}
			pageTemp++;
			blockcount++;
		}
		...
		if(pageTemp<=totalpages) {
			pagingStr += "<a href='"+reqUrl+"?pageNum="+pageTemp+"&ca="+ca+"'>[다음블록]</a>";
			pagingStr += "<a href='"+reqUrl+"?pageNum="+totalpages+"&ca="+ca+"'>[마지막페이지]</a>";
		}
