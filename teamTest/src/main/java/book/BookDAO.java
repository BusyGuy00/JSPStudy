	...
	// 검색 조건에 맞는 게시물의 개수를 반환 
		...
		//검색 조건에 맞는 게시물 목록을 반환 
			...
			public int updatePost(BookDTO dto) {
				int result = 0;
				String query = "update 책 set 이름=?, 저자=?, 출판사=? "
						+ ",출력이미지=?, 삽입이미지=?, 카테고리=?, 금액=?, 수량=?, 판매량=?, 리뷰수=? where 번호 =? ";
				//쿼리문 준비
			...
			//책 구매시 판매량 증가
			public int buybook(String idx) {
				int result = 0;
				String sql = "update 책 set 판매량 = 판매량+1, 수량 = 수량-1  where 번호="+idx;
				try {
					stmt = con.createStatement();
			...
			//리뷰수 업데이트
			public void upreview(String idx,int count) {
				String sql = "update 책 set 리뷰수="+count+" where 번호="+idx;
				try {
					stmt = con.createStatement();
