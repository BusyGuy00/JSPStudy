...
public class ViewController extends HttpServlet{ 
	...
	public void service(ServletRequest req, ServletResponse resp) throws ServletException, IOException {
		...
		//게시물 수 전달 받기 
		int totalCount = dao2.selectCount(map,idx);
		dao.upreview(idx, totalCount);
		dao.close();
		// 보이는 게시글 수 
		int pageSize = 10;
		int blockPage = 5;
		...
		//페이징 처리 end 
		//게시물 목록 받기 
		List<ReviewDTO> reviewlists = dao2.selectListPage(map,idx);
		dao2.close();
		
		//뷰에 전달할 매개변수 추가 " pageTemp 자리에 ./mvcboar/lost.do"를 대체해준다.
		String pagingStr = Paging.pagingstr(totalCount, pageSize, blockPage, pageNum, "view.do",idx);
