...
public class SearchAPI extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			//1 인증 정보 설정 
			String clientId = "lVY2kDrRVNILwPf_ZOfs"; //애플리케이션 클라이언트 아이디
	        String clientSecret = "aEt9h2N0AS"; //애플리케이션 클라이언트 시크릿

	        String text = null;
	        //2 검색 조건 설정 
		    int startNum =0; //검색 시작 위치
	        try {	//startNum은 인트 타입이여서 Integer로 담아준다.
	        	startNum = Integer.parseInt(req.getParameter("startNum"));
	        	//검색어를 매개 변수로 받겠다.
	        	String searchText = req.getParameter("keyword");
	        	//한글 깨짐을 방지하기 위해 UTF-8로 인고딩 해서 text변수에 넣어준다.
